...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if type(generation_config) != GenerationConfig:
            new_generation_config = GenerationConfig()
            for k, v in generation_config.items():
                setattr(new_generation_config, k, v)
        ...
        opera_decoding = generation_config.opera_decoding if generation_config.opera_decoding else None
        generation_config.output_attentions = True if generation_config.opera_decoding else None

        def get_index(img_mask):
            mask_int = img_mask.to(torch.int)
        ...
        if opera_decoding:
            ...
            if self.name_or_path == "Qwen/Qwen-VL-Chat":
                key_position = {
                    "image_start": int(((input_ids[0] == generation_config.img_start_id).nonzero(as_tuple=True))[0][0]), 
                    "image_end": int(((input_ids[0] == generation_config.img_end_id).nonzero(as_tuple=True))[0][0]), 
                    "response_start": len(input_ids[0]),
                    }
            elif self.name_or_path == 'internlm/internlm-xcomposer2-vl-7b':
                key_position = {
                    "image_start": int(((kwargs['im_mask'][0] == True).nonzero(as_tuple=True))[0][0]), 
                    "image_end": int(((kwargs['im_mask'][0] == True).nonzero(as_tuple=True))[0][-1]), 
                    "response_start": len(kwargs['im_mask'][0]),
                }
            elif self.name_or_path == 'llava-hf/llava-1.5-13b-hf':
                key_position = {
                    "image_start": int(((input_ids[0] == generation_config.image_token_id).nonzero(as_tuple=True))[0][0]), 
                    "image_end": int(((input_ids[0] == generation_config.image_token_id).nonzero(as_tuple=True))[0][0]) + 575, 
                    "response_start": len(input_ids[0])+575,
                }
            elif self.name_or_path == 'BAAI/Emu2-Chat':
                key_position = {
                    "image_start": int(((kwargs['input_dict']['selected'] == True).nonzero(as_tuple=True))[0][0]), 
                    "image_end": int(((kwargs['input_dict']['selected'] == True).nonzero(as_tuple=True))[0][-1]), 
                    "response_start": len(inputs_tensor[0]),
                    }
            elif "internlm2-chat-20b" in self.name_or_path:
                key_position = {
                    "image_start": int(((kwargs['input_dict']['selected'] == True).nonzero(as_tuple=True))[0][0]), 
                    "image_end": int(((kwargs['input_dict']['selected'] == True).nonzero(as_tuple=True))[0][-1]), 
                    "response_start": len(inputs_tensor[0]),
                    }
            elif "Yi-VL-34B" in self.name_or_path:
                key_position = {
                    "image_start": int(((input_ids[0] == generation_config.image_token_id).nonzero(as_tuple=True))[0][0]), 
                    "image_end": int(((input_ids[0] == generation_config.image_token_id).nonzero(as_tuple=True))[0][0]) + 1023, 
                    "response_start": len(input_ids[0])+1023,
                }
            elif self.name_or_path == "llava-hf/llava-v1.6-34b-hf":
                key_position = {
                    "image_start": int(((input_ids[0] == generation_config.image_token_id).nonzero(as_tuple=True))[0][0]), 
                    "image_end": int(((input_ids[0] == generation_config.image_token_id).nonzero(as_tuple=True))[0][0]), 
                    "response_start": len(input_ids[0]),
                }
        if generation_mode == GenerationMode.ASSISTED_GENERATION and opera_decoding:
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            next_token_scores_processed = logits_processor(input_ids, next_token_scores).to(device=input_ids.device)
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if not "past_key_values" in model_kwargs.keys():
                attn_previous = outputs.attentions[-1].clone().to(device=input_ids.device) # [batch_size * num_beams, num_head, q, kv]
            else:
                ...
                attn_previous = torch.cat(
                    [attn_previous[beam_idx], outputs.attentions[-1].clone().max(1, keepdim=True).values.data.to(device=input_ids.device)], -2) # [batch_size * num_beams, num_head, q, kv]
            ...
            next_token_logits = outputs.logits[:, -1, :].to(device=input_ids.device)
            ...
            for candidate_id in range(num_attn_candidates):
                ...
                attn_output = outputs_tmp.attentions[-1].clone().to(device=input_ids.device)
            ...
            candidate_token_scores -= penalty_weights * penalty_scores.to(device=input_ids.device)
            ...
            try:
                if all((rollback_loc_gather == rollback_loc).long().sum() > int(threshold) for _, rollback_loc_gather in enumerate(rollback_loc_gathers)):
                    ...
                    if self.name_or_path == 'pretrained/internlm2-chat-20b/':
                        answer_embeds = self.model.tok_embeddings(input_ids[:,:-1])
                        model_kwargs = model_kwargs_ori.copy()
                        model_kwargs["inputs_embeds"] = torch.cat([model_kwargs["inputs_embeds"], answer_embeds], 1)
                        model_kwargs["attention_mask"] = torch.cat(
                            [model_kwargs["attention_mask"].to(input_ids.device), torch.ones_like(input_ids[:,:-1]).to(input_ids.device)], 1)

                        model_inputs_tmp = self.prepare_inputs_for_generation(input_ids[:,:-1], **model_kwargs)
                    elif self.name_or_path == 'BAAI/Emu2-Chat':
                        answer_embeds = self.model.embed_tokens(input_ids[:,:-1])
                        model_kwargs = model_kwargs_ori.copy()
                        model_kwargs["inputs_embeds"] = torch.cat([model_kwargs["inputs_embeds"], answer_embeds], 1)
                        model_kwargs["attention_mask"] = torch.cat(
                            [model_kwargs["attention_mask"].to(input_ids.device), torch.ones_like(input_ids[:,:-1]).to(input_ids.device)], 1)
                        ...
                        model_kwargs["attention_mask"] = torch.cat(
                            [model_kwargs["attention_mask"].to(input_ids.device), torch.ones_like(input_ids[:,:-1]).to(input_ids.device)], 1)
                        model_kwargs["im_mask"] = torch.cat(
                            [model_kwargs["im_mask"].to(input_ids.device), torch.zeros_like(input_ids[:,:-1]).to(input_ids.device).bool()], 1)
                    ...
                    model_inputs_tmp['attention_mask'] = model_inputs_tmp['attention_mask'].int()
                    outputs_tmp = self(
                        **model_inputs_tmp,
                        return_dict=True,
                        output_attentions=output_attentions,
