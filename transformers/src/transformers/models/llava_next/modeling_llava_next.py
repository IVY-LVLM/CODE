...
class LlavaNextCausalLMOutputWithPast(ModelOutput):
    ...
    image_token_number: Optional[Tuple[torch.FloatTensor]] = None
...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel):
    ...
    def forward(
        ...
        image_token_number = 0
        if instead_pixel_values is not None:
            instead_pixel_embeds = self.get_input_embeddings()(instead_pixel_values)
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                if instead_pixel_values is not None:
                    image_features = instead_pixel_embeds
                
                inputs_embeds, attention_mask, labels, position_ids = self._merge_input_ids_with_image_features(
                    image_features, inputs_embeds, input_ids, attention_mask, labels
                )
                image_token_number = image_features.shape[1]
                if labels is None:
