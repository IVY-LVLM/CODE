...
from benchmarks.chair.bleu import bleu_four, bleu_one
...
class CHAIR(object):
    ...
    def __init__(self, coco_path, annotation):

        imids = list(annotation.keys())
        self.imid_to_segmentation = {imid: [] for imid in imids}
        self.imid_to_caption = {imid: [] for imid in imids}

        self.coco_path = coco_path

        #read in synonyms
        synonyms = open('benchmarks/chair/synonyms.txt').readlines()
    ...
    def get_annotations_from_segments(self):
        ...
        '''
        if os.path.exists('benchmarks/chair/coco_imid_to_objects_seg.pkl'):
            with open('benchmarks/chair/coco_imid_to_objects_seg.pkl', 'rb') as f:
                self.imid_to_segmentation = pickle.load(f)
            return
        ...
        for i, annotation in enumerate(segment_annotations):
            ...
            if imid in self.imid_to_segmentation:
                node_word = self.inverse_synonym_dict[id_to_name[annotation['category_id']]]
                self.imid_to_segmentation[imid].append(node_word)
        print ("\n")
        for imid in self.imid_to_segmentation:
            self.imid_to_segmentation[imid] = set(self.imid_to_segmentation[imid])
        
        with open('benchmarks/chair/coco_imid_to_objects_seg.pkl', 'wb') as f:
            pickle.dump(self.imid_to_segmentation, f)

    def get_annotations_from_captions(self):
        '''
        Add objects taken from MSCOCO ground truth captions 
        '''
        if os.path.exists('benchmarks/chair/coco_imid_to_objects.pkl'):
            with open('benchmarks/chair/coco_imid_to_objects.pkl', 'rb') as f:
                self.imid_to_caption = pickle.load(f)
            return
        ...
        for i, annotation in enumerate(caption_annotations):
            ...
            imid = annotation['image_id']
            if imid in self.imid_to_caption:
                self.imid_to_caption[imid].append(annotation['caption']) 
        print ("\n")

        for imid in self.imid_to_caption:
            self.imid_to_caption[imid] = set(self.imid_to_caption[imid])
        with open('benchmarks/chair/coco_imid_to_objects.pkl', 'wb') as f:
            pickle.dump(self.imid_to_caption, f)
    ...
    def compute_chair(self, prediction, annotation, image_id_list):
    
        '''
        Given ground truth objects and generated captions, determine which sentences have hallucinated words.
        '''
        imid_to_objects = self.imid_to_segmentation
        ...
        for i, (pred, anno, imid) in enumerate(zip(prediction, annotation, image_id_list)):
    
            cap = pred
            #get all words in the caption, as well as corresponding node word
            words, node_words, idxs, raw_words = self.caption_to_words(cap) 
            gt_cap = self.imid_to_caption[imid]
            gp_cap_list.append(gt_cap)
        ...
        bleu_4 = bleu_four(gp_cap_list, prediction)
        bleu_1 = bleu_one(gp_cap_list, prediction)
    ...
    def compute_only_chair(self, imids, generated_captions):
        ...
        for imid, gen_cap in zip(imids, generated_captions):

            # get all words in the caption, as well as corresponding node word
            words, node_words, idxs, raw_words = self.caption_to_words(gen_cap)
            ...
            for word, node_word, idx in zip(words, node_words, idxs):
                if node_word not in gt_objects:
                    ...
                    cap_dict['hallucination_idxs'].append(idx)
                    hallucinated = True
            for gt_word in cap_dict['mscoco_gt_words']:
                if gt_word not in cap_dict['mscoco_generated_words']:
                    cap_dict['missing_words'].append(gt_word)
...
def print_metrics(hallucination_cap_dict, quiet=False):
    ...
    metric_string = "%0.01f\t%0.01f" %(sentence_metrics['CHAIRs']*100, sentence_metrics['CHAIRi']*100)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--annotation_path", type=str, default='/mnt/hard1/yeonju/datasets/coco/annotations')
