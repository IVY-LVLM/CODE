...
context_file = "benchmarks/llava-qa90/caps_boxes_coco2014_val_80.jsonl"
compare_gpt4_file = "benchmarks/llava-qa90/qa90_gpt4_answer.jsonl"
rule_file = "benchmarks/llava-qa90/rule.jsonfile"
...
class LLaVAQA90Dataset(BaseEvalDataset):
    def __init__(self, data_path: str = "llava-qa90", default_output_path="LLaVA-QA90"):
        super().__init__("llava-qa90", data_path)
    ...
    def _evaluate(self, model, result_file_manager):
        ...
        # Question
        question_file = os.path.join(self.data_path, "qa90_questions.jsonl")
        ...
        for row_idx, line in enumerate(tqdm(questions, desc="LLaVA-QA90")):
            ...
            if self.result_file_manager.is_absent_sample(row_idx) == False:
                ...
                response = model.generate(question, image, dataset_name="llava-qa90", image_path = image_file)

                gpt_ans = gpt4_answers[idx]

                inst = image_to_context[image_file]

                if isinstance(inst['captions'], list):
                    cap_str = '\n'.join(inst['captions'])
                else:
                    cap_str = inst['captions']
