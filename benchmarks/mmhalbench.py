...
class MMHalBenchDataset(BaseEvalDataset):
    def __init__(self, data_path: str = "MMHALBENCH", split="test", default_output_path='MMHALBENCH'):
        super().__init__("mmhalbench", data_path)
    ...
    def _evaluate(self, model, result_file_manager):
        ...
        for row_idx, data in enumerate(tqdm(dataset['test'], desc="MMHal-Bench")):
            id = data['id']
            ...
            if self.result_file_manager.is_absent_sample(row_idx) == False:
                ...
                response = model.generate(cur_prompt, image, 'mmhalbench', image_path = image_path) 
                ...
                self.result_file_manager.save_result(image_id=image_id, question=question, gt_ans=correct_answer, pred_ans=pred_ans, raw_pred=raw_pred, mmvp_gpt_grade=gpt_grade, category_pope_mme=question_type, model_info=model_info)
