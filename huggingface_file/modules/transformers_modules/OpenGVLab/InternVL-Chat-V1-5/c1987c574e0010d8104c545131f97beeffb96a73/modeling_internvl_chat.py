...
class InternVLChatModel(PreTrainedModel):
    ...
    def chat(self, tokenizer, pixel_values, question, generation_config, history=None, return_history=False,
             IMG_START_TOKEN='<img>', IMG_END_TOKEN='</img>', IMG_CONTEXT_TOKEN='<IMG_CONTEXT>', input_dict = None, image_cd=None, cd_alpha=None):
    ...
    def generate(
        ...
        if pixel_values is not None:
            ...
            if input_dict:
                input_dict['selected'] = selected

            if image_cd is not None:
                import copy
                input_embeds_cd = copy.deepcopy(input_embeds)
                vit_embeds_cd = self.extract_feature(image_cd)
                input_embeds_cd[selected] = vit_embeds_cd.reshape(-1, C).to(input_embeds.device)
        ...
        outputs = self.language_model.generate(
            ...
            use_cache=True,
            input_dict=input_dict,
            cd_input_embed = input_embeds_cd,
            cd_alpha=cd_alpha,
            **generate_kwargs,
        )
