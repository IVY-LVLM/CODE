...
class InternLMXComposer2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def encode_img(self, image, vcd_decoding=False):
        if image is None:
            return None
        ...
        if vcd_decoding:
            image = add_diffusion_noise(image)
    ...
    def chat(
        ...
        **kwargs,
        ...
        if image is None:
            ...
            if kwargs['vcd_decoding']:
                image_cd = self.encode_img(image, vcd_decoding=True)
        ...
        if kwargs['vcd_decoding']:
            inputs_embed_cd, im_mask = self.interleav_wrap_chat(tokenizer, query, image_cd, history, meta_instruction)
            kwargs['cd_input_embed'] = inputs_embed_cd['inputs_embeds']
